...
const OsWiseTableGraphComponent = ({osWiseData,dates}) => {
    const [showTable, setShowTable] = React.useState(false);
    const [tablefields,setTableFields]=useState({
      field1:"Android Version",
    ...
    return<div className=" w-full bg-[#F2F2F2] mx-auto rounded-2xl overflow-x-auto">
        ...
        onClick={()=>{setShowTable(prev =>!prev)}}>
          <div className="px-4 py-5 flex flex-col gap-1">
              <div className="flex items-center justify-start gap-2">
                  ...
                  <div className="text-4xl font-semibold">OS Wise Installs</div>
              </div>
        ...
        <div className={`flex items-center justify-center overflow-y-auto ${(showTable)?"max-h-[500px]":"h-fit"} no-scrollbar`} ref={containerRef}>
          {(showTable)?<GenericTable tableData={osWiseData} tablehead={tablefields}/>:<GenericGraph graphData={osWiseData}/>}
        </div>
  </div>
}

export default OsWiseTableGraphComponent